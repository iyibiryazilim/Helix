...
namespace Helix.Queries
{
	public class BankAccountTransactionLineQuery : BaseQuery
	{
		...
		FROM LG_{FirmNumber.ToString().PadLeft(3, '0')}_{PeriodNumber.ToString().PadLeft(2, '0')}_BNFLINE AS BNFLINE
		LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_CLCARD AS CLCARD ON CLCARD.LOGICALREF = BNFLINE.CLIENTREF
		LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_BNCARD AS BNCARD ON BNCARD.LOGICALREF = BNFLINE.BANKREF
		LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_BANKACC AS BANKACC ON BANKACC.LOGICALREF = BNFLINE.BNACCREF
		LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_{PeriodNumber.ToString().PadLeft(2, '0')}_BNFICHE AS BNFICHE ON BNFICHE.LOGICALREF = BNFLINE.SOURCEFREF
