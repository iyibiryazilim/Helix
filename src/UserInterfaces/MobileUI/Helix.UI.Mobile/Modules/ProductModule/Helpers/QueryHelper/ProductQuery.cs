...
ORDER BY
    ...
    {
        var query = @$"SELECT 
        [Barcode] = (SELECT ISNULL(BARCODE,'') FROM LG_{CompanyNumber}_UNITBARCODE	WHERE ITEMREF = ITMUNITA.ITEMREF AND ITMUNITAREF = ITMUNITA.LOGICALREF AND UNITLINEREF = ITMUNITA.UNITLINEREF),
