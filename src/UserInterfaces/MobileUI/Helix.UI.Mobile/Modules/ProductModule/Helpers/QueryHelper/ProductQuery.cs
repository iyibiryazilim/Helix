...
{
	static string CompanyNumber = string.Empty;
	static string CompanyPeriod = string.Empty;
	...
	public string DetailValues(int ReferenceId)
	{
		string query = $@"SELECT
               ...
               [StockQuantity] = (SELECT ISNULL(SUM(ONHAND), 0) FROM LV_{CompanyNumber}_{CompanyPeriod}_STINVTOT AS STINVTOT LEFT JOIN L_CAPIWHOUSE AS WAREHOUSE ON STINVTOT.INVENNO = WAREHOUSE.NR AND WAREHOUSE.FIRMNR = {Int32.Parse(SecureStorage.Default.GetAsync("companyNumber").Result)}
