...
public class WastageTransactionDataStore : IWastageTransactionService
{
    ...
    public async Task<DataResult<WastageTransactionDto>> InsertObject(HttpClient httpClient, WastageTransactionDto wastageTransactionDto)
    {
        HttpResponseMessage responseMessage = await httpClient.PostAsync(postUrl, new StringContent(JsonSerializer.Serialize(wastageTransactionDto), Encoding.UTF8, "application/json"));

        DataResult<WastageTransactionDto> dataResult = new DataResult<WastageTransactionDto>();
        ...
        if (dataResult.IsSuccess)
        {
            ...
            {
                var result = JsonSerializer.Deserialize<DataResult<WastageTransactionDto>>(data, new JsonSerializerOptions
                {
                    PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                });
                dataResult.Data = result?.Data;
