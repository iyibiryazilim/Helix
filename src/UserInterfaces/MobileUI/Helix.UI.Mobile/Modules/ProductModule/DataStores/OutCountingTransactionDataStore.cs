...
public class OutCountingTransactionDataStore : IOutCountingTransactionService
{
    ...
    public async Task<DataResult<OutCountingTransactionDto>> InsertObject(HttpClient httpClient, OutCountingTransactionDto outCountingTransactionDto)
    {
        HttpResponseMessage responseMessage = await httpClient.PostAsync(postUrl, new StringContent(JsonSerializer.Serialize(outCountingTransactionDto), Encoding.UTF8, "application/json"));

        DataResult<OutCountingTransactionDto> dataResult = new DataResult<OutCountingTransactionDto>();
        ...
        if (dataResult.IsSuccess)
        {
            ...
            {
                var result = JsonSerializer.Deserialize<DataResult<OutCountingTransactionDto>>(data, new JsonSerializerOptions
                {
                    PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                });
                dataResult.Data = result?.Data;
