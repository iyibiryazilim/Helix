...
public class WholeSalesReturnDispatchTransactionDataStore : IWholeSalesReturnDispatchTransactionService
{
    ...
    public async Task<DataResult<WholeSalesReturnTransactionInsertDto>> InsertObject(HttpClient httpClient, WholeSalesReturnTransactionInsertDto salesReturnTransactionInsertDto)
    {
        HttpResponseMessage responseMessage = await httpClient.PostAsync(postUrl, new StringContent(JsonSerializer.Serialize(salesReturnTransactionInsertDto), Encoding.UTF8, "application/json"));

        DataResult<WholeSalesReturnTransactionInsertDto> dataResult = new DataResult<WholeSalesReturnTransactionInsertDto>();
        ...
        if (dataResult.IsSuccess)
        {
            ...
            {
                var result = JsonSerializer.Deserialize<DataResult<WholeSalesReturnTransactionInsertDto>>(data, new JsonSerializerOptions
                {
                    PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                });
                dataResult.Data = result?.Data;
