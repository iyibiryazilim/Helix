...
{
    public partial class ReturnByPurchaseDispatchTransactionLineSupplierViewModel : BaseViewModel
    {
		...
		public ReturnByPurchaseDispatchTransactionLineSupplierViewModel(IHttpClientService httpClientService, ISupplierService supplierService)
		{
		...
		async Task GoToFicheListAsync()
		{
			try
			{
				if (Items.Where(c => c.IsSelected).Any())
				{
					...
					await Task.Delay(500);
					await Shell.Current.GoToAsync($"{nameof(ReturnByPurchaseDispatchTransactionLineShipInfoListView)}", new Dictionary<string, object>
					{
