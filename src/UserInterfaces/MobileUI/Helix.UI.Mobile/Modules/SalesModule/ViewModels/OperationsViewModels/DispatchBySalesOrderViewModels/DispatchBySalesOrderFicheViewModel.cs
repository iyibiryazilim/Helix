...
public partial class DispatchBySalesOrderFicheViewModel : BaseViewModel
{
    ...
    async Task GetSalesOrdersAsync()
    {
        ...
        try
        {
            ...
            {
                var result = await _salesOrderService.GetObjectsByCurrentIdAndWarehouseNumberAndShipInfo(httpClient, Current.ReferenceId, Warehouse.Number, ShipInfo.ReferenceId, SearchText, OrderBy, CurrentPage, PageSize);
                Items.Clear();
                Results.Clear();
                foreach (SalesOrder item in result.Data)
                {
        ...
        try
        {
            if (!string.IsNullOrEmpty(text))
            {
                if (text.Length >= 3)
                {
                    ...
                    Results.Clear();
                    foreach (var item in Items.ToList().Where(x => x.Code.ToLower().Contains(SearchText) || x.CurrentName.ToLower().Contains(SearchText) || x.CurrentCode.ToLower().Contains(SearchText)))
                    {
                        Results.Add(item);
                    }
                }
