...
public partial class PurchaseDispatchFormViewModel : BaseViewModel
{
	...
	public PurchaseDispatchFormViewModel(IHttpClientService httpClientService, IWarehouseService warehouseService, ISupplierService supplierService, ISpeCodeService speCodeService, IPurchaseDispatchTransactionService purchaseDispatchTransactionService, IServiceProvider serviceProvider)
	{
	...
	async Task PurchaseDispatchInsert()
	{
		try
		{
			...
			if (result.IsSuccess)
			{
				var viewModel = _serviceProvider.GetService<PurchaseDispatchListViewModel>();
