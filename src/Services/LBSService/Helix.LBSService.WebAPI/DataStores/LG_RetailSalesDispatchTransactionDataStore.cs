...
namespace Helix.LBSService.WebAPI.DataStores
{
	public class LG_RetailSalesDispatchTransactionDataStore : ILG_RetailSalesDispatchTransactionService
	{
		...
		public async Task<DataResult<RetailSalesDispatchTransactionDto>> Insert(RetailSalesDispatchTransactionDto dto)
		{
			...
			DataResult<RetailSalesDispatchTransactionDto> result = new();
			var transaction = Mapping.Mapper.Map<LG_RetailSalesDispatchTransaction>(dto);

			foreach (var line in dto.Lines.ToList())
			{
				var transactionLine = Mapping.Mapper.Map<LG_RetailSalesDispatchTransactionLine>(line);
			...
			if (unity.LoggedIn)
			{
				...
				catch (Exception ex)
				{
					...
					result = new DataResult<RetailSalesDispatchTransactionDto>
					{
						Data = null,
						IsSuccess = false,
						Message = ex.Message
