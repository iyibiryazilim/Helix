...
{
	public class LG_PurchaseReturnDispatchTransactionDataStore : ILG_PurchaseReturnDispatchTransactionService
	{
		IUnityApplicationService _unityApplicationService;

		public LG_PurchaseReturnDispatchTransactionDataStore(IUnityApplicationService unityApplicationService)
		{
		...
		public async Task<DataResult<PurchaseReturnDispatchTransactionDto>> Insert(PurchaseReturnDispatchTransactionDto dto)
		{
			...
			DataResult<PurchaseReturnDispatchTransactionDto> result = new();
			var transaction = Mapping.Mapper.Map<LG_PurchaseReturnDispatchTransaction>(dto);

			foreach (var line in dto.Lines.ToList())
			{
				var transactionLine = Mapping.Mapper.Map<LG_PurchaseReturnDispatchTransactionLine>(line);
			...
			if (unity.LoggedIn)
			{
				...
				catch (Exception ex)
				{
					...
					result = new DataResult<PurchaseReturnDispatchTransactionDto>
					{
						Data = null,
						IsSuccess = false,
						Message = ex.Message
