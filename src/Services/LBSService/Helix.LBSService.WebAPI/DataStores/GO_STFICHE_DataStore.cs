...
{
	public class GO_STFICHE_DataStore
	{
		public async ValueTask<string> GetLastAsgn(string effsdate, string effedate, int TRCODE)
		{
			var lastNumber = string.Empty;
			string query = $"SELECT TOP 1 LASTASGND as lastNumber FROM L_LDOCNUM WHERE EFFSDATE<='{effsdate}' AND EFFEDATE>='{effedate}' AND FIRMID={LBSParameter.FirmNumber} AND DOCIDEN = {TRCODE}";
			await using (SqlConnection connection = new SqlConnection(LBSParameter.Connection))
			{
        ...
        WHERE EFFSDATE<='{effsdate}' AND EFFEDATE>='{effedate}' AND FIRMID={LBSParameter.FirmNumber} AND DOCIDEN ={TRCODE} ";
			await using (SqlConnection connection = new SqlConnection(LBSParameter.Connection))
			{
				await connection.OpenAsync();
				await using (SqlCommand command = new SqlCommand(query, connection))
		...
		public async Task<string> InsertObject(LG_STFICHE item)
		{
				...
				try
				{
					...
					await using (SqlConnection connection = new SqlConnection(LBSParameter.Connection))
					{
						...
						await using (SqlCommand command = new SqlCommand(query, connection))
						{
							...
							command.Parameters.AddWithValue("FTIME", item.FTIME);
					...
					if (item.TRCODE == 2 || item.TRCODE == 3 || item.TRCODE == 7 || item.TRCODE == 8)
					{
						...
						string driverQuery = string.Format(@$"INSERT INTO LG_{LBSParameter.FirmNumber.ToString().PadLeft(3, '0')}_{LBSParameter.Period.ToString().PadLeft(2, '0')}_EINVOICEDET
            ([INVOICEREF]
           ,[STFREF]
           ,[EINVOICETYP]
           ,[PROFILEID]
           ...
           ,@SELLERCLIENTREF,@TAXNRTOPAY); SELECT SCOPE_IDENTITY();");
					...
					{
						string lineQuery = $@"INSERT INTO LG_{LBSParameter.FirmNumber.ToString().PadLeft(3, '0')}_{LBSParameter.Period.ToString().PadLeft(2, '0')}_STLINE(
            STOCKREF
           ,LINETYPE
           ,PREVLINEREF
           ,PREVLINENO
           ...
           ,@FUTMONTHBEGDATE
						...
						await using (SqlConnection connection = new SqlConnection(LBSParameter.Connection))
						{
							...
							try
							{
								await using (SqlCommand command = new SqlCommand(lineQuery, connection))
								{
									...
									command.Parameters.AddWithValue("FTIME", line.FTIME);
						...
						if (line.SERILOTTRANSACTIONS != null)
						{
							...
							{
								string seriLotQuery = $@"INSERT INTO LG_{LBSParameter.FirmNumber.ToString().PadLeft(3, '0')}_{LBSParameter.Period.ToString().PadLeft(2, '0')}_SLTRANS(
            STFICHEREF
           ,DATE_
           ,STTRANSREF
           ,INTRANSREF
