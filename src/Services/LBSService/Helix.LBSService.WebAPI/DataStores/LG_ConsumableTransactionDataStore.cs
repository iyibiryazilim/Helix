...
namespace Helix.LBSService.WebAPI.DataStores
{
	public class LG_ConsumableTransactionDataStore : ILG_ConsumableTransactionService
	{
		...
		public async Task<DataResult<ConsumableTransactionDto>> Insert(ConsumableTransactionDto dto)
		{
			...
			{
				var transactionLine = Mapping.Mapper.Map<LG_ConsumableTransactionLine>(line);
				...
				try
				{
					if (unity != null)
					{
						if (unity.LoggedIn)
						{
							...
							foreach (LG_ConsumableTransactionLine line in transaction.TRANSACTIONS)
							{
			...
			{
				result = new DataResult<ConsumableTransactionDto>
				{
					Data = null,
					IsSuccess = false,
