...
namespace Helix.LBSService.Go.DataStores
{
	public class LG_STFICHE_Context : ILG_STFICHE_Context
	{
		...
		public async Task<int> UpdateObject(SqlConnection connection, string lastAsgnd, string effsdate, string effedate, int TRCODE)
		{
			...
			using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
			{
				try
				{
					...
					await using (SqlConnection connection = new SqlConnection(_connectionString))
					{
						...
						#region Line Insert
						ReferenceId = await CreateLine(item, ReferenceId, connection);
						#endregion

						await CreateDriver(item, ReferenceId, connection);

						await UpdateObject(connection, last, item.DATE_.ToString("s"), item.DATE_.ToString("s"), item.TRCODE);
					}
					#endregion 
		...
		private async Task CreateDriver(LG_STFICHE item, int ReferenceId, SqlConnection connection)
		{
		...
		private async Task<int> CreateLine(LG_STFICHE item, int ReferenceId, SqlConnection connection)
		{
           ...
           ,@FUTMONTHBEGDATE
				...
				try
				{
					...
					{
						ReferenceId = await CreateSerilot(ReferenceId, connection, line);
		...
		private async Task<int> CreateSerilot(int ReferenceId, SqlConnection connection, LG_STLINE line)
		{
		...
		public async Task<DataResult<LG_STFICHE>> InsertTransferTransaction(LG_STFICHE item)
		{
         ...
         VALUES(
           ...
           ,@EXPDATE
					...
					#endregion
					await UpdateObject(null,last, item.DATE_.ToString("s"), item.DATE_.ToString("s"), item.TRCODE);
					transaction.Complete();
