...
namespace Helix.LBSService.EventConsumer.ProductTransaction
{
	public class ConsumableTransactionConsumer : IDisposable
	{
		...
		private string _queueName = "ProductService.ConsumableTransactionIns"; //gonna change
		private string _exchange = "HelixTopicName";

		public ConsumableTransactionConsumer(ILG_ConsumableTransactionService consumableTransactionService, HttpClient httpClient)
		{
		...
		private async Task<bool> PostDtoToApiAsync(ConsumableTransactionDto dto)
		{
			try
			{
		...
		private void GetMessageFromQueue()
		{
			try
			{
				...
				ConsumableTransactionDto dto = new ConsumableTransactionDto();
				...
				consumer.Received += async (model, ea) =>
				{
					try
					{
						...
						dto = JsonConvert.DeserializeObject<ConsumableTransactionDto>(message);
