...
namespace Helix.LBSService.Tiger.DataStores
{
	public class LG_WholeSalesReturnDispatchTransactionDataStore : ILG_WholeSalesReturnDispatchTransactionService
	{
		...
		public LG_WholeSalesReturnDispatchTransactionDataStore(IUnityApplicationService unityApplicationService)
		{
		...
		public async Task<DataResult<WholeSalesReturnTransactionDto>> Insert(WholeSalesReturnTransactionDto dto)
		{
			...
			DataResult<WholeSalesReturnTransactionDto> result = new();
			var transaction = Mapping.Mapper.Map<LG_WholeSalesReturnDispatch>(dto);

			foreach (var line in dto.Lines.ToList())
			{
				var transactionLine = Mapping.Mapper.Map<LG_WholeSalesReturnDispatchLine>(line);
			...
			if (unity.LoggedIn)
			{
				...
				catch (Exception ex)
				{
					...
					result = new DataResult<WholeSalesReturnTransactionDto>
					{
						Data = null,
						IsSuccess = false,
						Message = ex.Message
