...
namespace Helix.LBSService.Tiger.DataStores
{
	public class LG_TransferTransactionDataStore : ILG_TransferTransactionService
	{
		...
		public async Task<DataResult<TransferTransactionDto>> Insert(TransferTransactionDto dto)
		{
			UnityApplication unity = Global.UnityApp;
			DataResult<TransferTransactionDto> result = new();
			var transaction = Mapping.Mapper.Map<LG_TransferTransaction>(dto);

			foreach (var line in dto.Lines.ToList())
			{
				var transactionLine = Mapping.Mapper.Map<LG_TransferTransactionLine>(line);
			...
			if (unity.LoggedIn)
			{
				...
				try
				{
					if (unity != null)
					{
						if (unity.LoggedIn)
						{
							...
							foreach (LG_TransferTransactionLine line in transaction.TRANSACTIONS)
							{
				...
				{
					result = new DataResult<TransferTransactionDto>
					{
						Data = null,
						IsSuccess = false,
						Message = ex.Message
