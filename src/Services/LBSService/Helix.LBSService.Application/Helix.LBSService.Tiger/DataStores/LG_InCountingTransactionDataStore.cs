...
{
	public class LG_InCountingTransactionDataStore : ILG_InCountingTransactionService
	{
		IUnityApplicationService _unityApplicationService;

		public LG_InCountingTransactionDataStore(IUnityApplicationService unityApplicationService)
		{
		...
		public async Task<DataResult<InCountingTransactionDto>> Insert(InCountingTransactionDto dto)
		{
			...
			DataResult<InCountingTransactionDto> result = new();
			var transaction = Mapping.Mapper.Map<LG_InCountingTransaction>(dto);

			foreach (var line in dto.Lines.ToList())
			{
				var transactionLine = Mapping.Mapper.Map<LG_InCountingTransactionLine>(line);
			...
			if (unity.LoggedIn)
			{
				...
				try
				{
					if (unity != null)
					{
						if (unity.LoggedIn)
						{
							...
							foreach (LG_InCountingTransactionLine line in transaction.TRANSACTIONS)
							{
				...
				{
					result = new DataResult<InCountingTransactionDto>
					{
						Data = null,
						IsSuccess = false,
