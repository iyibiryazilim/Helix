...
namespace Helix.LBSService.Tiger.DataStores
{
	public class LG_PurchaseReturnDispatchTransactionDataStore : ILG_PurchaseReturnDispatchTransactionService
	{
		...
		IEventBus _eventBus;
		public LG_PurchaseReturnDispatchTransactionDataStore(IUnityApplicationService unityApplicationService, IEventBus eventBus)
		{
			_unityApplicationService = unityApplicationService;
			_eventBus = eventBus;
		}
		public async Task<DataResult<LG_PurchaseReturnDispatchTransaction>> Insert(LG_PurchaseReturnDispatchTransaction dto)
		{
			UnityApplication unity = Global.UnityApp;
			DataResult<LG_PurchaseReturnDispatchTransaction> result = new();
			...
			if (unity.LoggedIn)
			{
				...
				{
					result = new DataResult<LG_PurchaseReturnDispatchTransaction>
					{
						Data = null,
						IsSuccess = false,
						Message = ex.Message
