...
namespace Helix.LBSService.Tiger.DataStores
{
	public class LG_ProductionTransactionDataStore : ILG_ProductionTransactionService
	{
		...
		public async Task<DataResult<ProductionTransactionDto>> Insert(ProductionTransactionDto dto)
		{
			...
			DataResult<ProductionTransactionDto> result = new();
			var transaction = Mapping.Mapper.Map<LG_ProductionTransaction>(dto);

			foreach (var line in dto.Lines.ToList())
			{
				var transactionLine = Mapping.Mapper.Map<LG_ProductionTransactionLine>(line);
			...
			if (unity.LoggedIn)
			{
				...
				try
				{
					if (unity != null)
					{
						if (unity.LoggedIn)
						{
							...
							foreach (LG_ProductionTransactionLine line in transaction.TRANSACTIONS)
							{
				...
				{
					result = new DataResult<ProductionTransactionDto>
					{
						Data = null,
						IsSuccess = false,
						Message = ex.Message
