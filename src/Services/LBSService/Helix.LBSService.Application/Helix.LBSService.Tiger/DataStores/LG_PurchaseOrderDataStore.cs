						...
						if (unity != null)
						{
							if (unity.LoggedIn)
							{
								...
								foreach (LG_PurchaseOrderLine line in dto.TRANSACTIONS)
								{
									...
									//	dtos_lines[dtos_lines.Count - 1].FieldByName("GUID").Value = dto.GUID.ToString();
									//	dtos_lines[dtos_lines.Count - 1].FieldByName("PARENTLNREF").Value = 332293;
									//}
									dtos_lines.AppendLine();
