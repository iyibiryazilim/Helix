...
{
	public class PurchaseOrderInsertingIntegrationEventHandler : IIntegrationEventHandler<PurchaseOrderInsertingIntegrationEvent>
	{
		private readonly IHttpClientService _httpClientService;
		private readonly ILogger<OutCountingTransactionInsertingIntegrationEventHandler> _logger;

		public PurchaseOrderInsertingIntegrationEventHandler(IHttpClientService httpClientService, ILogger<OutCountingTransactionInsertingIntegrationEventHandler> logger)
		{
			_httpClientService = httpClientService;
			_logger = logger;
		}

		public async Task Handle(PurchaseOrderInsertingIntegrationEvent @event)
		{
			var httpClient = _httpClientService.GetOrCreateHttpClient();
			try
			{
				string apiUrl = "/api/PurchaseOrder/Insert"; // Replace with your actual API endpoint
