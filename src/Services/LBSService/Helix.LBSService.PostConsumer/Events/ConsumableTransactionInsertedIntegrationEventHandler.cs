...
namespace Helix.LBSService.PostConsumer.Events
{
	public class ConsumableTransactionInsertedIntegrationEventHandler : IIntegrationEventHandler<ConsumableTransactionInsertedIntegrationEvent>
	{
		...
		readonly ILogger<ConsumableTransactionInsertedIntegrationEventHandler> _logger;
        public ConsumableTransactionInsertedIntegrationEventHandler(IHttpClientService httpClientService, ILogger<ConsumableTransactionInsertedIntegrationEventHandler> logger)
		{
			_httpClientService = httpClientService;
			_logger = logger;
		}
		public async Task Handle(ConsumableTransactionInsertedIntegrationEvent @event)
		{
			var httpClient = _httpClientService.GetOrCreateHttpClient();
			try
			{
				string apiUrl = ""; // Replace with your actual API endpoint 
