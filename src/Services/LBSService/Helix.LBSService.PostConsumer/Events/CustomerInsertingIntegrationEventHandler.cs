...
namespace Helix.LBSService.PostConsumer.Events
{
	public class CustomerInsertingIntegrationEventHandler : IIntegrationEventHandler<CustomerInsertingIntegrationEvent>
	{
		...
		public CustomerInsertingIntegrationEventHandler(IHttpClientService httpClientService, ILogger<ConsumableTransactionInsertingIntegrationEventHandler> logger)
		{
			_httpClientService = httpClientService;
			_logger = logger;
		}

		public async Task Handle(CustomerInsertingIntegrationEvent @event)
		{
			var httpClient = _httpClientService.GetOrCreateHttpClient();
			try
			{
				string apiUrl = "/api/Current/Insert"; // Replace with your actual API endpoint
