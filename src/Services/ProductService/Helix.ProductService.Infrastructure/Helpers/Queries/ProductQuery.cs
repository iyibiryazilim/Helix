...
namespace Helix.ProductService.Infrastructure.Helpers.Queries
{
	public class ProductQuery : BaseQuery
    {
        ...
        public string GetAlternativeProductList(int id,string search = "", string orderBy = ProductOrderBy.ItemNameAsc, int page = 0, int pageSize = 20)
        {
            ...
            [LastTransactionDate] =(SELECT TOP 1 DATE_ FROM LG_{FirmNumber.ToString().PadLeft(3, '0')}_{PeriodNumber.ToString().PadLeft(2, '0')}_STLINE WHERE STOCKREF = ITEMS.LOGICALREF ORDER BY  DATE_ DESC) 
			...
			LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_FIRMDOC AS FIRMDOC ON FIRMDOC.INFOREF = ITEMS.LOGICALREF AND FIRMDOC.INFOTYP = 20
            LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_UNITSETF AS UNITSET ON ITEMS.UNITSETREF = UNITSET.LOGICALREF 
			LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_UNITSETL AS UNITSETL ON UNITSET.LOGICALREF = UNITSETL.UNITSETREF
        ...
        public string GetCustomerAndSupplierList(int id, string search = "", string orderBy = ProductOrderBy.ItemNameAsc, int page = 0, int pageSize = 20)
        {
		    ...
		    FROM LG_{FirmNumber.ToString().PadLeft(3, '0')}_SUPPASGN AS SUPPASGN
		    LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_CLCARD AS CLCARD ON SUPPASGN.CLIENTREF = CLCARD.LOGICALREF
		    LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_ITEMS AS ITEMS ON SUPPASGN.PACKETREF = ITEMS.LOGICALREF AND ITEMS.PACKET = 1
		    LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_UNITSETF AS UNITSET ON ITEMS.UNITSETREF = UNITSET.LOGICALREF
		    LEFT JOIN LG_{FirmNumber.ToString().PadLeft(3, '0')}_UNITSETL AS UNITSETL ON UNITSETL.UNITSETREF = UNITSET.LOGICALREF AND UNITSETL.MAINUNIT = 1
        ...
        public string GetProductsGroups()
        {
            string query = @$"SELECT
                ...
                from LG_{FirmNumber.ToString().PadLeft(3, '0')}_SPECODES as specode
