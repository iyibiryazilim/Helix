...
namespace Helix.ProductionService.WebAPI.Controllers
{
	...
	public class WorkOrderController : ControllerBase
	{
		...
		{
			_eventBus.Publish(new WorkOrderInsertActualQuantityIntegrationEvent(workOrderDto.eventId, workOrderDto.workOrderReferenceId, workOrderDto.productReferenceId, workOrderDto.actualQuantity, workOrderDto.subUnitsetReferenceId, workOrderDto.calculatedMethod, workOrderDto.isIncludeSideProduct));
		}
		...
		{
			_eventBus.Publish(new WorkOrderChangeStatusInsertedIntegrationEvent(workOrderChangeStatusDto.eventId, ficheNo: workOrderChangeStatusDto.ficheNo,
		...
		{
			_eventBus.Publish(new WorkOrderStopTransactionInsertingIntegrationEvent(stopTransactionForWorkOrderDto.eventId, stopTransactionForWorkOrderDto.workOrderReferenceId, stopTransactionForWorkOrderDto.stopCauseReferenceId, stopTransactionForWorkOrderDto.stopDate, stopTransactionForWorkOrderDto.stopTime));
		}
	}
}