...
public class SupplierQuery : BaseQuery
{
	...
	public string GetSupplierList(string search, string orderBy, int currentPage = 0, int pageSize = 20)
    {
        ...
        [ReferenceCount]=ISNULL((SELECT COUNT(DISTINCT STOCKREF) FROM LG_00{FirmNumber}_0{PeriodNumber}_ORFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND (AMOUNT-SHIPPEDAMOUNT) > 0 AND CLOSED = 0  AND LINETYPE = 0 AND TRCODE = 2 ),0),
		[NetTotal] = ISNULL((SELECT SUM(LINENET) FROM LG_00{FirmNumber}_0{PeriodNumber}_ORFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND(AMOUNT - SHIPPEDAMOUNT) > 0 AND CLOSED = 0 AND LINETYPE = 0 AND TRCODE = 2),0),
